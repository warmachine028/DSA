# CODE 9b: Optimization of 0-1 Knapsack problem with Memoization


def knapsack(max_capacity: int,  # Time Complexity: O(max_capacity * n)   Space Complexity: (max_capacity * n)
             weights: list[int],
             values: list[int], memo=None) -> int:
    n = len(weights)
    key = max_capacity, n
    ...
    if weights[n - 1] > max_capacity:
        memo[key] = knapsack(max_capacity, weights[:-1], values[:-1])
        return memo[key]
    
    # Recursive case
    memo[key] = max(values[-1] + knapsack(max_capacity - weights[-1], weights[:-1], values[:-1], memo),  # Take the item
                    #                                                                  so item weight must be subtracted
                    knapsack(max_capacity, weights[:-1], values[:-1], memo)  # don't take the item
                    )
    return memo[key]
