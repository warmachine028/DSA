# CODE 9a: Given two integer arrays A and B of size N each which represent values and weights associated with N items
# respectively. Also given an integer C which represents knapsack capacity. Find out the maximum value subset of A such
# that sum of the weights of this subset is smaller than or equal to C.
#
# NOTE:
# You cannot break an item, either pick the complete item, or don’t pick it (0-1 property).

def knapsack(max_capacity: int,  # Time Complexity: O(2^n)  Space Complexity: O(1)
             weights: list[int], values: list[int]) -> int:
    n = len(weights)
    # Base Cases
    if 0 in (n, max_capacity):
        return 0
    if weights[n - 1] > max_capacity:
        return knapsack(max_capacity, weights[:-1], values[:-1])
    
    # Recursive case
    return max(
        values[n - 1] + knapsack(max_capacity - weights[n - 1], weights[:-1], values[:-1]),  # We take the item
        knapsack(max_capacity, weights[:-1], values[:-1])  # We don't take the item
    )
...
print(knapsack(841,
               [4, 4, 5, 2, 2, 4, 9, 8, 5, 3, 8, 8, 10, 4, 2, 10, 9, 7, 6, 1, 3, 9, 7, 1, 3, 5, 9, 7, 6, 1, 10,
                1, 1, 7, 2, 4, 9, 10, 4, 5, 5, 7],
