# CODE 9c: Optimization of 0-1 Knapsack problem with tabulation

def knapsack(max_capacity: int,  # Time Complexity: O(n * max_weight)  Space Complexity: O(n * weight)
             weights: list[int], values: list[int]) -> int:
    table = [[0] * (max_capacity + 1) for _ in range(len(weights) + 1)]
    for capacity in range(1, max_capacity + 1):
        for weight in range(1, len(weights) + 1):
            if weights[weight - 1] <= capacity:
                table[weight][capacity] = table[weight - 1][capacity - weights[weight - 1]] + values[weight - 1]
            table[weight][capacity] = max(table[weight][capacity], table[weight - 1][capacity])
    
    return table[len(weights)][max_capacity]
